...
// UIEditorManaged.cpp : Defines the entry point for the DLL application.
//
...
VIMPORT IVisPlugin_cl* GetEnginePlugin_UIEnginePlugin();
...
namespace UIEditorManaged
{
  ...
  public:
    ...
    static void InitManagedModule()
    {
      ...
      GetEnginePlugin_UIEnginePlugin()->InitEnginePlugin();

	  
	  VUINodeMananger_cl::GlobalManager()->SetEditorMode( true );
	  VUINodeMananger_cl::GlobalManager()->InitializeUIEngine();
	

	   UIEditorMessageService::Initialize();
    }

    static void DeInitManagedModule()
    {
      //Deinit the entity plugin directly since we link statically to it
      GetEnginePlugin_UIEnginePlugin()->DeInitEnginePlugin();

	  UIEditorMessageService::Deinitialize();
