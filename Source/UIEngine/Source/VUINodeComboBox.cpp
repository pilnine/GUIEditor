#include "VUINodeComboBox.h"


V_IMPLEMENT_SERIAL( VUINodeComboBox, VPushButton, 0, &g_NodeModule );

VUINodeComboBox::VUINodeComboBox(void)
{
...
VUINodeComboBox* VUINodeComboBox::CreateInstance()
{
	// CreateInstance
	VUINodeComboBox* pNewPushButton = new VUINodeComboBox();
...
void VUINodeComboBox::SetItemList ( VArray<VString> itemList )
{
	m_ItemList = itemList;
}
...
void VUINodeComboBox::Serialize( VArchive &ar )
{
...
bool VUINodeComboBox::Build(TiXmlElement *pNode, const char *szPath, bool bWrite) 
{
	if ( bWrite == false )
	{		
...
void VUINodeComboBox::OnVariableValueChanged(VisVariable_cl *pVar, const char * value)
{
	VASSERT (pVar);
	VASSERT (value);
...
START_VAR_TABLE(VUINodeComboBox, VisTypedEngineObject_cl,"animated entity", VVARIABLELIST_FLAGS_NONE, NULL );
...
DEFINE_VAR_INT_AND_NAME(VUINodeComboBox, m_eTransp, "Transparency" ,  "Transparency", "VIS_TRANSP_ALPHATEST", 0, "CSharpType(TransparencyType)");
...
//DEFINE_VAR_STRING(VUINodeComboBox, m_sUIKey, "Path key to follow", "", 128, 0, 0);
END_VAR_TABLE
